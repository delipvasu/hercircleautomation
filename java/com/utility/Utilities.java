...
//	final static Logger logger = Logger.getLogger("rootLogger");
	...
	public boolean JSClick(By byLocator, String text) {
		...
		js.executeScript("findElement(byLocator).value='text'");
		return true;
...
//		}
	...
	public boolean verifyElementDisplayed(By byLocator) throws Exception {
		try {
			WebElement element = findElement(byLocator);
			if (element.isDisplayed()) {
				System.out.println("elemeent is displayed");
				return true;
			}
		} catch (Exception e) {
			return false;
	...
	public void loadingpage(By locator ,int waittime)
	{
    new WebDriverWait(getDriver(), waittime).until(ExpectedConditions.invisibilityOfElementLocated(locator));

	}
	public void username(By byLocator,String text, String FieldName, ResourceBundle rb)
	{
		
		type(byLocator, text, FieldName,rb);
	
	}
	public void Password(By byLocator,String text, String FieldName, ResourceBundle rb)
	{
	...
	public void click1(By byLocator) throws Exception
	{
...
//		waitTime(8000);
	...
	public static boolean verifyIsElementDisplayed(By by) {
		...
		System.out.println("find the element"+list);
		getDriver().manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
		if (list.size() == 0) {
			return false;
		} else {
	 ...
	 * @param rb 
	 */
	public void type(By byLocator, String text, String FieldName, ResourceBundle rb) {

		try {
			WebElement element = findElement(byLocator);
			element.sendKeys(rb.getString(text)); 
			text = text.split("\n")[0];
		...
		try {
			if (getPlatform().equals("Web")) {
				...
				for (int i = 0; i < x; i++) {
					...
					logger.info("Back button is tapped");
					extent.extentLogger("Back", "Andriod Back button is tapped");
				}
			}
		} catch (Exception e) {
			logger.error(e);
