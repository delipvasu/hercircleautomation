...
//	final static Logger logger = Logger.getLogger("rootLogger");
	...
	public void onStart(ITestContext context) {
		...
		test.set(extent.get().createTest(getClass().getName()));
...
//			ExcelUpdate.creatExcel();
			childTest.set(test.get().createNode(result.getMethod().getMethodName()));
		} else {
			runmode = false;
...
//		MailReport.EmailReport();
	...
	public void appVersion() {
		if (getPlatform().equals("Android")) {
			...
			setAppVersion("Build " + DeviceDetails.getAppVersion(handler.getproperty("eonPackage")).trim()
					.replace("versionName=", ""));
